# test-demo — Specialized Space — domain-specific functionality
...
3. **Не чіпай інші простори** — працюй тільки в межах ~/spaces/test-demo/
4. 5. **Доменна експертиза** — працюй тільки в межах домену test-demo.
...
**Колекція**: `space_test-demo` на vuzol:6333 (авто-створюється при першому push)

### Перед роботою — пошук контексту

```bash
# Знайти релевантні знання в пам'яті простору
ssh vuzol python3 /root/scripts/memory-to-qdrant.py --search "ключові слова" --space test-demo
...
git add -A && git commit -m "memory(test-demo): опис змін" && git push vuzol main
```
...
cwd → ~/spaces/test-demo/    (щоб цей CLAUDE.md авто-завантажився)
