...
**Перед роботою**: `ssh vuzol python3 /root/scripts/memory-to-qdrant.py --search "запит" --space legal
**Після роботи**: зберегти в `~/spaces/legal/memory/agents/doc-reviewer/<name>.md` → git push
**Колекція**: `agent_legal_doc-reviewer`
**PG лог**: `ssh vuzol python3 /root/scripts/agent-log.py --space legal --agent doc-reviewer --status done --model MODEL --tokens IN,OUT --cost USD --duration MS --summary "що зроблено"`
