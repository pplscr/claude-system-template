...
| `CROSS-SPACE.md` | Міжпросторова комунікація — `cat CROSS-SPACE.md` |
...
3. **Не чіпай інші простори** — працюй тільки в межах ~/spaces/{SPACE_NAME}/
4. {SPACE_SPECIFIC_RULES}
...
**Колекція**: `space_{SPACE_NAME}` на vuzol:6333 (авто-створюється при першому push)

### Перед роботою — пошук контексту

```bash
# Знайти релевантні знання в пам'яті простору
ssh vuzol python3 /root/scripts/memory-to-qdrant.py --search "ключові слова" --space {SPACE_NAME}
...
git add -A && git commit -m "memory({SPACE_NAME}): опис змін" && git push vuzol main
```
...
cwd → ~/spaces/{SPACE_NAME}/    (щоб цей CLAUDE.md авто-завантажився)
